...
class Network:
    def __init__(self, vocab, properties):
        ...
        self.tag_embedding = self.model.add_lookup_parameters((vocab.num_pos(), properties.pos_embed_dim))
        ...
        self.hidden_layer_1 = self.model.add_parameters((properties.hidden1_dim, self.input_dim))

        # define the hidden layer bias term and initialize it as constant 0.2.
        self.hidden_layer_1_bias = self.model.add_parameters(properties.hidden1_dim, init=dynet.ConstInitializer(0.2))

        # define the hidden layer.
        self.hidden_layer_2 = self.model.add_parameters((properties.hidden2_dim, properties.hidden1_dim))

        # define the hidden layer bias term and initialize it as constant 0.2.
        self.hidden_layer_2_bias = self.model.add_parameters(properties.hidden2_dim, init=dynet.ConstInitializer(0.2))
    ...
    def build_graph(self, features):
        ...
        tag_ids     = [self.vocab.pos2id(tag_feat) for tag_feat in features[20:40]]
        ...
        hidden1 = self.transfer(self.hidden_layer_1.expr() * embedding_layer + self.hidden_layer_1_bias.expr())
        hidden2 = self.transfer(self.hidden_layer_2.expr() * hidden1 + self.hidden_layer_2_bias.expr())
